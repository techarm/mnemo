...
知識をベクトル DB に記録する。`reference` 型は Web 調査結果を TTL 付きで保存。`procedure` 型はセットアップ手順を永続保存（信頼度減衰・TTL 対象外）。

| パラメータ | 型 | 必須 | 説明 |
|-----------|---|------|------|
| type | enum | ✓ | lesson / pitfall / preference / pattern / solution / reference / procedure |
| title | string | ✓ | 短いタイトル |
...
| rawContent | string | | 取得した全文テキスト（reference/procedure 用。embedding には含めない） |
...
| type | enum | | タイプでフィルタ（reference/procedure 含む） |
...
| スキル | コマンド | 概要 |
|-------|---------|------|
| learn | `/learn` | エラーや教訓から知識を抽出・記録（reference/procedure 型対応） |
| research | `/research` | Web Search + Context7 で調査し、結果を reference として永続化 |
| setup | `/setup` | ツールのセットアップ手順を調査・構造化・procedure として保存。既存手順がある場合は実行モード |
