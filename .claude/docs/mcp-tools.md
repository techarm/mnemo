...
Mnemo のデュアルインターフェース。MCPサーバー（Claude Code 向け）と CLI（ターミナル向け）が同じ core 層を共有し、同一機能を提供する。MCPツール（10個）は `src/index.ts`、CLI は `bin/mnemo.ts` に定義。
...
| パラメータ | 型 | 必須 | 説明 |
|-----------|---|------|------|
| type | enum | ✓ | lesson / pitfall / preference / pattern / solution |
| title | string | ✓ | 短いタイトル |
...
ハイブリッド検索（セマンティック + キーワード）で知識を検索する。

| パラメータ | 型 | 必須 | 説明 |
|-----------|---|------|------|
| query | string | ✓ | 検索クエリ（自然言語 or キーワード） |
| type | enum | | タイプでフィルタ |
...
| フック | トリガー | 動作 |
|-------|---------|------|
| session-start.sh | セッション開始時 | mnemo recall で関連知識を自動表示 |
